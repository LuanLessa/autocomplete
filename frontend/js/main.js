import AutoCompleteSystem from './core/AutoCompleteSystem.js';
...
const api = new SyncService(API_URL);
...
document.getElementById('btn-login').addEventListener('click', async () => {
    ...
    if (!usernameInput) return alert("Digite um usuário!");

    try {
        // 1. Chama o servidor
        const data = await api.login(usernameInput);
        userId = data.userId;

        // 2. Inicializa o Sistema com o ID recebido
        acSystem = new AutoCompleteSystem(userId);
        ...
        // 4. Carrega eventos da UI agora que o sistema existe
        iniciarApp();
...
// --- LÓGICA DO APP (Encapsulada numa função) ---

async function iniciarApp() {
    await acSystem.loadFromDB();
